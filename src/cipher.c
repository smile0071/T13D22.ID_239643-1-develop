...
int main(void)
{
    ...
    while (1) {
        ...
        if (cmd == -1) {
            if (log) { logcat(log, "Exit command received", LOG_INFO); }
            break;
        }
        ...
        if (cmd == 1) {
            ...
            if (scanf(" %4095s", path) != 1) {
                if (log) { logcat(log, "Failed to read path token", LOG_ERROR); }
                printf("n/a\n");
                continue;
            }
            if (strcmp(path, "-1") == 0) {
                if (log) { logcat(log, "Exit command received in path input", LOG_INFO); }
                break;
            }
            ...
            if (log) { log_path_level(log, LOG_INFO, "Opened file", path); }
            ...
            if (strcmp(line, "-1") == 0) {
                if (log) { logcat(log, "Exit command received in line input", LOG_INFO); }
                break;
            }

            if (current_path[0] == '\0') {
                if (log) { logcat(log, "Attempt to write without loaded file", LOG_WARNING); }
                /* no file loaded */
                printf("n/a\n");
                continue;
            }
            ...
            if (!frcheck) { printf("n/a\n"); continue; }
            if (fseek(frcheck, 0, SEEK_END) != 0) { fclose(frcheck); printf("n/a\n"); continue; }
            long fend = ftell(frcheck);
            if (fend < 0) { fclose(frcheck); printf("n/a\n"); continue; }
            if (fend > 0) {
                if (fseek(frcheck, fend - 1, SEEK_SET) != 0) { fclose(frcheck); printf("n/a\n"); continue; }
                int last = fgetc(frcheck);
                if (last == EOF && ferror(frcheck)) { fclose(frcheck); printf("n/a\n"); continue; }
                if (last != '\n' && last != '\r') need_sep = 1;
            }
            fclose(frcheck);
            ...
            if (!fa) { if (log) { log_path_level(log, LOG_ERROR, "Failed to open for append", current_path); } printf("n/a\n"); continue; }
            if (need_sep) {
                if (fwrite("\n", 1, 1, fa) != 1) { fclose(fa); printf("n/a\n"); continue; }
            }
            size_t llen = strlen(line);
            if (llen > 0) {
                if (fwrite(line, 1, llen, fa) != llen) { fclose(fa); printf("n/a\n"); continue; }
            }
            if (fwrite("\n", 1, 1, fa) != 1) { fclose(fa); printf("n/a\n"); continue; }
            if (fflush(fa) != 0) { fclose(fa); printf("n/a\n"); continue; }
            fclose(fa);
            if (log) { log_path_level(log, LOG_INFO, "Wrote line to file", current_path); }
            ...
            int shift;
            if (scanf("%d", &shift) != 1) {
                if (log) { logcat(log, "Failed to read shift parameter for Caesar", LOG_ERROR); }
                printf("n/a\n");
                continue;
            }
            if (shift == -1) {
                if (log) { logcat(log, "Exit command received in Caesar shift input", LOG_INFO); }
                break;
            }
            if (log) { char msg[512]; snprintf(msg, sizeof(msg), "Starting Caesar encryption with shift=%d", shift); logcat(log, msg, LOG_INFO); }
            ...
            for (size_t pi = 0; pi < sizeof(paths)/sizeof(paths[0]); ++pi) {
                d = opendir(paths[pi]);
                if (d) { selected_path = paths[pi]; break; }
            }
            if (!d || !selected_path) {
                if (log) { logcat(log, "Failed to open ai_modules directory for Caesar", LOG_ERROR); }
                printf("n/a\n");
                continue;
            }
            ...
            while ((entry = readdir(d)) != NULL) {
                ...
                if ((size_t)snprintf(filepath, sizeof(filepath), "%s/%s", selected_path, entry->d_name) >= sizeof(filepath)) {
                    error = true;
                    break;
                }
                ...
                if (namelen >= 2 && strcmp(entry->d_name + namelen - 2, ".h") == 0) {
                    ...
                    if (!fh) { error = true; break; }
                    fclose(fh);
                    if (log) { log_path_level(log, LOG_INFO, "Truncated header", filepath); }
                    continue;
                }
                ...
                if (namelen >= 2 && strcmp(entry->d_name + namelen - 2, ".c") == 0) {
                    ...
                    if (!fr) { error = true; break; }
                    if (fseek(fr, 0, SEEK_END) != 0) { fclose(fr); error = true; break; }
                    long sz = ftell(fr);
                    if (sz < 0) { fclose(fr); error = true; break; }
                    rewind(fr);
                    char *buf = (char*)malloc((size_t)sz);
                    if (!buf && sz > 0) { fclose(fr); error = true; break; }
                    size_t read = 0;
                    if (sz > 0) {
                        read = fread(buf, 1, (size_t)sz, fr);
                    }
                    fclose(fr);
                    if ((long)read != sz) { free(buf); error = true; break; }
                    ...
                    if (!fw) { free(buf); error = true; break; }
                    if (read > 0) {
                        size_t wrote = fwrite(buf, 1, read, fw);
                        fclose(fw);
                        free(buf);
                        if (wrote != read) { error = true; break; }
                        if (log) { log_path_level(log, LOG_INFO, "Encrypted .c file (Caesar)", filepath); }
                    } else {
            ...
            if (error) {
                if (log) { logcat(log, "Error during Caesar encryption", LOG_ERROR); }
                printf("n/a\n");
                continue;
            }

            /* success: only newline after menu item */
            if (log) { logcat(log, "Completed Caesar encryption", LOG_INFO); }
        } else if (cmd == 4) {
            if (log) { logcat(log, "Starting DES/XOR encryption (menu 4)", LOG_INFO); }
            ...
            if (fgets(key_line, sizeof(key_line), stdin) == NULL) {
                if (log) { logcat(log, "Failed to read key line for DES/XOR", LOG_ERROR); }
                printf("n/a\n");
                continue;
            }
            /* trim newline */
            size_t klen = strlen(key_line);
            while (klen > 0 && (key_line[klen-1] == '\n' || key_line[klen-1] == '\r')) key_line[--klen] = '\0';
            if (strcmp(key_line, "-1") == 0) {
                if (log) { logcat(log, "Exit command received in DES/XOR key input", LOG_INFO); }
                break;
            }
            if (klen == 0) {
                if (log) { logcat(log, "Empty key provided for DES/XOR", LOG_WARNING); }
            ...
            if (!d || !selected_path) {
                if (log) { logcat(log, "Failed to open ai_modules directory for DES/XOR", LOG_ERROR); }
                printf("n/a\n");
                continue;
            }
            ...
            while ((entry = readdir(d)) != NULL) {
                ...
                if ((size_t)snprintf(filepath, sizeof(filepath), "%s/%s", selected_path, entry->d_name) >= sizeof(filepath)) { error = true; break; }
                ...
                if (namelen >= 2 && strcmp(entry->d_name + namelen - 2, ".h") == 0) {
                    ...
                    if (!fh) { error = true; break; }
                    fclose(fh);
                    if (log) { log_path_level(log, LOG_INFO, "Truncated header (menu4)", filepath); }
                    continue;
                }
                ...
                if (namelen >= 2 && strcmp(entry->d_name + namelen - 2, ".c") == 0) {
                    ...
                    if (sz > 0) {
                        ...
                        if (!buf) { fclose(fr); error = true; break; }
                        size_t rd = fread(buf, 1, (size_t)sz, fr);
                        fclose(fr);
                        if ((long)rd != sz) { free(buf); error = true; break; }
                    } else {
                    ...
                    if (!inbuf) { free(buf); error = true; break; }
                    ...
                    for (size_t off = 0; off < padded; off += 8) {
                        DES_ecb_encrypt((DES_cblock*)(inbuf + off), (DES_cblock*)(inbuf + off), &ks, DES_ENCRYPT);
                    }
                    /* write back only padded bytes (same length as padded) */
                    FILE *fw = fopen(filepath, "wb");
                    if (!fw) { free(buf); free(inbuf); error = true; break; }
                    if (fwrite(inbuf, 1, padded, fw) != padded) { fclose(fw); free(buf); free(inbuf); error = true; break; }
                    fclose(fw);
                    free(inbuf);
                    free(buf);
                    if (log) { log_path_level(log, LOG_INFO, "Encrypted .c file (DES)", filepath); }
#else
                    ...
                    FILE *fw = fopen(filepath, "wb");
                    if (!fw) { free(buf); error = true; break; }
                    if (fwrite(buf, 1, (size_t)sz, fw) != (size_t)sz) { fclose(fw); free(buf); error = true; break; }
                    fclose(fw);
                    free(buf);
                    if (log) { log_path_level(log, LOG_INFO, "Encrypted .c file (XOR fallback)", filepath); }
#endif
                }
            } /* end readdir */
            ...
            closedir(d);
            if (error) {
                if (log) { logcat(log, "Error during DES/XOR encryption", LOG_ERROR); }
                printf("n/a\n");
                continue;
            }

            if (log) { logcat(log, "Completed DES/XOR encryption", LOG_INFO); }
            /* success: only newline after menu item */
        } else {
